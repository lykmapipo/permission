...
const PERMISSION_MODEL_NAME =
  getString('PERMISSION_MODEL_NAME', 'Permission');
const PERMISSION_COLLECTION_NAME =
  getString('PERMISSION_COLLECTION_NAME', 'permissions');
const PERMISSION_SEED = getString('PERMISSION_SEED', 'permissions');
const SCHEMA_OPTIONS =
  ({ timestamps: true, emitIndexErrors: true, collection: PERMISSION_COLLECTION_NAME });
...
PermissionSchema.statics.seed = function seed(seeds, done) {
  ...
  PERMISSION_SEED_ACTIONS =
    ([].concat(DEFAULT_ACTIONS).concat(PERMISSION_SEED_ACTIONS));
  PERMISSION_SEED_ACTIONS = _.uniq(_.compact(PERMISSION_SEED_ACTIONS));
  ...
  resources = (
    []
    .concat(resources)
    .concat(mongoose.modelNames())
    .concat(PERMISSION_SEED_RESOURCES)
  );
